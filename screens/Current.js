			...
			<SafeAreaView
				...
				<GooglePlacesAutocomplete
					...
					onPress={(data, details = null) => {
						// fix google names
						var updatedAddress = details.formatted_address.replace(
							/^[\s\d]+/,
							''
						);
						this.setState({
					...
					<CollapseBody
						...
						<View style={{ padding: 10 }}>
							<Text style={currentStyles.currentDescSummaryBold}>
								<Ionicons name='md-time' size={19} color={colours.white} />{' '}
								Daily Summary
							</Text>
							<Text style={currentStyles.currentDescSummary}>
								Feels like {Math.round(this.props.feelslike)}°,{' '}
								{this.props.desc} with {Math.round(this.props.wind)} km/h wind,{' '}
								{this.props.humidity}% humidity and an expected high of{' '}
								{this.props.high}°
							</Text>
							{/* sun details */}
