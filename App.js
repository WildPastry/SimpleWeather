...
class App extends Component {
	...
	getSkyData() {
		...
		Promise.all([
			...
			fetch(
				openCurrentURL + myLat + '&lon=' + myLng + '&units=metric&APPID=' + open
			),
			fetch(
				openWeekURL + myLat + '&lon=' + myLng + '&units=metric&APPID=' + open
			),
		])
			// set json
	...
	setBgDay() {
		...
		if (currentID >= 200 && currentID <= 232) {
			...
			// group 3xx: drizzle
		} else if (currentID >= 300 && currentID <= 321) {
